		...
		Collections.sort(this.allUserList, new Comparator<HibUserMasterModel>() {
			public int compare(HibUserMasterModel u1, HibUserMasterModel u2) {
				...
				if (("ユーザーID").equals(sortColumn)) {
					...
					return invertFlag * (u1.getHibProfileInfoModel().getUserName()
							.compareTo(u2.getHibProfileInfoModel().getUserName()) >= 0 ? 1 : -1);
					...
					return invertFlag
							* (u1.getHibProfileInfoModel().getTel().compareTo(u2.getHibProfileInfoModel().getTel()) >= 0
									? 1
									: -1);
				} else if (("郵便番号").equals(sortColumn)) {
					return invertFlag * (u1.getHibProfileInfoModel().getPostcode()
							.compareTo(u2.getHibProfileInfoModel().getPostcode()) >= 0 ? 1 : -1);
				} else if (("住所").equals(sortColumn)) {
					return invertFlag * (u1.getHibProfileInfoModel().getAddress()
							.compareTo(u2.getHibProfileInfoModel().getAddress()) >= 0 ? 1 : -1);
				} else if (("コメント").equals(sortColumn)) {
					return invertFlag * (u1.getHibProfileInfoModel().getComment()
							.compareTo(u2.getHibProfileInfoModel().getComment()) >= 0 ? 1 : -1);
				} else {
					return invertFlag * 1;
				}
			}
